...
const CAT_ICONS={'2d':'▭','3d':'◈','trig':'sin','alg':'xⁿ','seq':'Σ','code':'{}','bits':'01','stats':'σ','calc':'∫','cplx':'O(n)','coord':'⊕','matrix':'[M]'};
...
function renderSection(sec){
  ...
  if(sec.big)h+=`<div class="fbig">${esc(sec.big)}</div>`;
  if(sec.formulas)sec.formulas.forEach(f=>{h+=`<div class="fr">${esc(f)}</div>`;});
  if(sec.table){
...
function setupPWA(){
  try{
    ...
    const manifest={
      ...
      description:'150+ math formulas across 12 categories. Works offline.',
...
(function initMobileSheet(){
  ...
  document.getElementById('ms-fab').classList.add('fab-visible');
...
function showMobileSheet(){
  ...
  document.getElementById('ms-fab').classList.remove('fab-visible');
}
...
function hideMobileSheet(permanent){
  ...
  bd.classList.remove('ms-open');
  if(permanent===true)sessionStorage.setItem('ms-dismissed','1');
  setTimeout(()=>{
    if(!sheet.classList.contains('ms-open')){
      bd.style.display='none';
    }
  },420);
  // Show FAB after dismissal so user can re-open
  if(!permanent){
    setTimeout(()=>document.getElementById('ms-fab').classList.add('fab-visible'),500);
  } else {
    hideFAB();
  }
}
